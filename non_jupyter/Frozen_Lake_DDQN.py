...
def optimize_model():
    ...
    next_state_final_mask = torch.tensor(tuple(map(lambda d: d in [5, 7, 11, 12],
                                                   batch.next_state)), device=device, dtype=torch.bool).unsqueeze(1)
    ...
    next_state_values[next_state_final_mask] = torch.zeros(len(next_state_finak_list), device=device).view(
        len(next_state_finak_list))
    #     print("next_state_values")
    #     print(next_state_values.shape)
...
for k in range(NUM_EPISODES):
    ...
    while not done:
        ...
        if sample >= eps_threshold:
            ...
            action = LongTensor([[random.randint(0, 3)]])
